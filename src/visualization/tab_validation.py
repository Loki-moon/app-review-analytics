...
def _render_summary(vr: dict[str, Any]) -> None:
    ...
    _summary_tooltips = {
        "모형 적합도":   "Pseudo R² — 모형이 데이터를 얼마나 잘 설명하는지 (≥0.2 양호)",
        "회귀계수 유의성": "p<0.05 기능 비율 — 우연이 아닌 실제 영향 키워드 비율 (≥50% 양호)",
        "상호작용 검정":  "LR test — 앱 간 기능 영향력 차이가 통계적으로 유의한지",
        "공출현 패턴":   "스피어만 r — 기능 키워드가 같은 리뷰에서 함께 등장하는 정도 (|r|<0.7 양호)",
        "평점 민감도":   "3점 처리 방식 변경 시 OR 방향 유지율 (≥80% 양호)",
        "기간 안정성":   "상반기·하반기 OR 방향 일치율 — 시간에 따른 결과 안정성 (≥80% 양호)",
        "표본 분포":    "긍정/부정 비율 균형 — 극단적 불균형 시 분석 편향 우려",
    }
    for label, status in items:
        bg = {"pass": "#065F46", "warn": "#92400E", "fail": "#B91C1C"}[status]
        icon = {"pass": "✅", "warn": "⚠️", "fail": "❌"}[status]
        tip = _summary_tooltips.get(label, "")
        st.markdown(f"""
        <span style="background:{bg};color:white;padding:4px 12px;border-radius:99px;
                     font-size:0.8rem;font-weight:600;">{icon} {label}</span>
        <span style="font-size:0.72rem;color:#94A3B8;margin-left:2px;margin-right:8px;">{tip}</span>
        """, unsafe_allow_html=True)
...
def _render_model_fit(vr: dict[str, Any]) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    로지스틱 회귀모형이 리뷰 데이터를 얼마나 잘 설명하는지 평가합니다.
    모형이 데이터를 잘 설명한다 = 기능 키워드가 사용자 평점에 실제로 영향을 준다는 뜻입니다.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · <b>Pseudo R²</b> (맥파든 R²): "이 모형이 아무것도 모르는 모형보다 얼마나 더 잘 맞추는가"를 0~1로 표현합니다.
      일반 R²와 달리 0.2~0.4도 매우 우수한 수준이에요.<br>
    &nbsp;&nbsp;&nbsp;— ❌ &lt;0.1: 경고 (기능 키워드가 평점을 거의 설명하지 못함)<br>
    &nbsp;&nbsp;&nbsp;— ⚠️ 0.1~0.2: 주의 (기본적인 경향 파악 가능)<br>
    &nbsp;&nbsp;&nbsp;— ✅ ≥0.2: 양호 (신뢰할 수 있는 설명력)<br>
    · <b>AIC / BIC</b> (정보 기준): 모형의 "경제성"을 측정합니다. 낮을수록 군더더기 없이 데이터를 잘 설명합니다.
      여러 앱을 비교할 때 상대적 비교 지표로 활용하세요.<br>
    · <b>Log-Likelihood</b>: 모형이 실제 데이터를 관측할 확률의 로그값 — 클수록(0에 가까울수록) 좋습니다.<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    Pseudo R²가 높을수록 "이 앱에서 특정 기능 키워드가 평점을 좌우하는 핵심 요소"라는 주장을 뒷받침합니다.
    </div>
    """, unsafe_allow_html=True)
...
def _render_coef_sig(vr: dict[str, Any]) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    각 기능 키워드가 평점에 미치는 영향이 "우연"이 아닌 "실제 효과"인지 확인합니다.
    샘플이 크면 작은 효과도 우연히 유의하게 나올 수 있어 반드시 확인이 필요합니다.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · <b>β (베타)</b>: 해당 기능이 언급되면 로그오즈(긍정 확률의 로그비)가 얼마나 변하는지<br>
    · <b>OR (오즈비, Odds Ratio)</b>: β의 지수 변환. OR=2.0이면 그 기능이 언급된 리뷰가
      언급되지 않은 리뷰보다 긍정 평점일 확률이 2배라는 뜻.<br>
    &nbsp;&nbsp;&nbsp;— OR &gt; 1: 긍정 평점과 연관 (클수록 긍정 영향 강함)<br>
    &nbsp;&nbsp;&nbsp;— OR = 1: 평점과 무관<br>
    &nbsp;&nbsp;&nbsp;— OR &lt; 1: 부정 평점과 연관 (작을수록 부정 영향 강함)<br>
    · <b>95% CI (신뢰구간)</b>: 실제 OR이 이 범위 안에 있을 확률 95%. 구간이 1을 포함하지 않아야 유의합니다.<br>
    · <b>p-value</b>: 이 결과가 우연일 확률. p&lt;0.05이면 "95% 신뢰 수준에서 우연이 아니다"라고 판단합니다.<br>
    · <b>Wald 검정</b>: 개별 회귀계수가 0인지(= 효과 없는지) 검정하는 방법 — p-value 계산의 기반<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    차트에서 <b>채색된 점</b>이 유의한 기능(p&lt;0.05), <b>회색 점</b>이 비유의 기능입니다.
    OR=1 점선 기준으로 오른쪽이면 긍정, 왼쪽이면 부정 연관입니다.
    신뢰구간(가로 막대)이 점선에 걸치지 않을수록 결과가 확실합니다.
    </div>
    """, unsafe_allow_html=True)
    ...
    _thesis_box(
        "각 기능 카테고리별 개별 로지스틱 회귀모형에서 Wald 검정을 통해 회귀계수의 통계적 유의성을 확인하였다. "
        "회귀계수(β)를 지수 변환한 오즈비(OR)와 95% 신뢰구간을 Forest Plot으로 시각화하였으며, "
        "p &lt; 0.05인 기능 키워드를 유의한 예측변수로 간주하였다. "
        "OR &gt; 1인 기능 키워드는 해당 기능이 언급된 리뷰에서 긍정 평점(4~5점)의 오즈가 높음을 의미하며, "
        "OR &lt; 1인 기능 키워드는 부정 평점(1~2점)과의 연관성을 나타낸다. "
        "신뢰구간이 OR=1을 포함하지 않는 경우 통계적으로 유의하다고 판단하였다."
    )
...
def _render_interaction(vr: dict[str, Any], combined_or: pd.DataFrame) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    "결제 기능에 대한 불만이 A앱에서 더 심각한가?"처럼, 같은 기능이어도 앱마다 평점에 미치는 영향이
    달라야 경쟁사 비교 분석이 의미를 가집니다. 이 검정은 그 차이가 우연이 아닌지 확인합니다.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · <b>Wald 검정</b>: 상호작용 계수(기능 × 앱 더미)가 0인지 검정. p&lt;0.05이면 앱 간 영향력 차이가 유의합니다.<br>
    · <b>LR 검정 (우도비 검정, Likelihood Ratio Test)</b>: 상호작용항을 추가했을 때 모형이 얼마나 개선되는지 비교.
      p&lt;0.05이면 상호작용 포함 모형이 통계적으로 더 우수합니다.<br>
    · <b>ΔOR</b>: 두 앱 간 오즈비 차이. 양수이면 기준 앱이 비교 앱 대비 해당 기능에서 더 높은 긍정 연관을 가집니다.<br>
    &nbsp;&nbsp;&nbsp;— *** p&lt;0.001 · ** p&lt;0.01 · * p&lt;0.05<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    ΔOR이 크고 유의한 기능일수록 앱 간 차별적 개선 포인트가 됩니다.
    유의한 쌍이 없다면 두 앱이 해당 기능에서 사용자 반응이 비슷하다는 뜻입니다.
    3개 이상 앱 분석 시 모든 쌍(A-B, A-C, B-C)에 대해 개별 검정을 수행하여 정확한 쌍별 차이를 파악합니다.
    </div>
    """, unsafe_allow_html=True)
    ...
    if isinstance(ia, pd.DataFrame) and not ia.empty:
        ...
        st.info("상호작용 검정은 2개 이상의 앱을 동시에 분석할 때 제공됩니다. "
                "왼쪽 사이드바에서 비교할 앱을 추가로 선택하면 앱 간 기능 영향력 차이를 검정할 수 있습니다.")

    _thesis_box(
        "서비스 간 기능 영향력의 차이가 통계적으로 유의한지 검증하기 위해, "
        "서비스 쌍(pair)별로 기능 키워드 × 서비스 더미 변수의 상호작용항을 포함한 모형과 제외한 모형을 비교하는 "
        "우도비 검정(Likelihood Ratio Test, LR test)과 Wald 검정을 병행하였다. "
        "분석 대상이 3개 이상의 서비스인 경우, 가능한 모든 쌍 조합(A-B, A-C, B-C 등)에 대해 개별 검정을 실시하여 "
        "편향 없는 쌍별 비교가 가능하도록 하였다. "
        "각 기능에서 가장 유의한 쌍의 LR 검정 결과를 대표값으로 보고하였으며(p &lt; 0.05 기준), "
        "유의한 상호작용이 확인된 기능에 한하여 오즈비 차이(ΔOR)를 경쟁사 간 비교 지표로 활용하였다."
    )
...
def _render_multicollinearity(vr: dict[str, Any]) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    "결제 불만"과 "보안 불만"이 항상 같은 리뷰에서 등장한다면, 두 기능의 OR이 독립적인 의미를
    가지는지 의심해볼 필요가 있습니다. 이 섹션은 그런 "함께 언급되는 패턴"을 사전에 파악합니다.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · <b>스피어만 상관계수 (Spearman r)</b>: 두 기능 키워드의 등장 빈도가 함께 높아지고 낮아지는 정도를 -1~1로 표현합니다.
      연속 데이터가 아닌 이진(있다/없다) 데이터에 적합한 비모수 상관 지표입니다.<br>
    &nbsp;&nbsp;&nbsp;— r이 높으면: 같은 리뷰에서 두 기능이 자주 함께 언급된다는 뜻<br>
    &nbsp;&nbsp;&nbsp;— OR 해석 시 참고: 예를 들어 "결제"와 "보안"의 r=0.75이면, 두 기능의 OR을 각각 해석할 때
      "이 두 기능은 함께 경험되는 경향이 있다"는 점을 언급하는 것이 좋습니다<br>
    · <b>|r| ≥ 0.7 (빨간 테두리)</b>: 강한 공출현 — 해석 시 반드시 주의 필요<br>
    · <b>|r| 0.4~0.7</b>: 중간 공출현 — 관련성 언급 권장<br>
    · <b>|r| &lt; 0.4</b>: 약한 공출현 — 각 기능 독립적으로 해석 가능<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    본 연구는 기능별 <b>개별 회귀</b> 설계를 채택하였으므로, 동일 모형 내 다중공선성은 구조적으로 발생하지 않습니다.
    (VIF 미산출이 정당한 이유입니다.) 스피어만 r은 OR 해석의 보조 맥락 지표로 활용하세요.
    </div>
    """, unsafe_allow_html=True)
    ...
    if not corr_matrix.empty:
        ...
        st.info("공출현 패턴 계산에 필요한 데이터가 부족합니다. "
                "기능 키워드가 등장하는 리뷰가 충분히 수집되면 히트맵이 표시됩니다.")

    _thesis_box(
        "본 연구는 기능 카테고리별 개별 로지스틱 회귀를 채택하였으므로, 동일 모형 내 "
        "복수 기능 예측변수가 공존하지 않아 다중공선성은 구조적으로 발생하지 않는다. "
        "이에 분산팽창인수(VIF) 산출은 적용하지 않으며, 기능 키워드 간 공출현 경향을 파악하기 위해 "
        "이진 더미 변수에 적합한 스피어만(Spearman) 순위 상관계수 행렬을 보조 지표로 보고하였다(Hauke &amp; Kossowski, 2011). "
        "스피어만 r이 높은 쌍은 같은 리뷰에서 두 기능이 자주 함께 언급되는 경향이 있으며, "
        "해당 기능의 OR 해석 시 이 공출현 맥락을 함께 고려하였다. "
        "|r| ≥ 0.7인 쌍은 강한 공출현으로 판단하여 OR 단독 해석을 지양하고 함께 논의하였다."
    )
...
def _render_threshold_sensitivity(vr: dict[str, Any]) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    리뷰 평점을 "긍정/부정"으로 나눌 때 3점을 어떻게 처리하느냐에 따라 결과가 달라질 수 있습니다.
    "3점을 제외한다", "3점을 긍정으로 본다", "3점을 부정으로 본다" — 세 가지 방식 모두에서
    같은 방향(긍정 OR 또는 부정 OR)이 나온다면 분석이 기준 선택에 민감하지 않다는 뜻이에요.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · 차트의 <b>3개 선</b>이 같은 방향(OR=1 기준으로 모두 위 또는 모두 아래)으로 움직이면 ✅<br>
    · <b>X 표시된 기능</b>: 이분화 기준에 따라 OR 방향이 바뀌는 불안정한 기능 — 해석 주의<br>
    · <b>방향 일치율 ≥ 80%</b>: 양호 — "3점 처리 방식과 무관하게 결과가 안정적이다"고 주장 가능<br>
    · <b>방향 일치율 &lt; 80%</b>: 주의 — 이분화 기준 선택의 근거를 논문에 명시적으로 기술 필요<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    이 검증은 논문 심사에서 자주 나오는 "왜 3점을 그렇게 처리했나요?" 질문에 대한 데이터 기반 답변입니다.
    </div>
    """, unsafe_allow_html=True)
...
def _render_period_stability(vr: dict[str, Any]) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    "상반기에 대규모 업데이트가 있었다면, 그 시기의 리뷰가 전체 결과를 왜곡하지 않았을까?"라는
    질문에 답합니다. 시간적 편향 없이 결과가 안정적임을 보여주는 핵심 강건성 검증입니다.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · <b>히트맵 색상</b>: 초록색(OR&gt;1, 긍정 연관) ↔ 빨간색(OR&lt;1, 부정 연관)<br>
    · <b>색이 일관되면(전체·상반기·하반기 모두 같은 색)</b>: 시간과 무관하게 결과가 안정적 ✅<br>
    · <b>빨간 테두리</b>: 기간에 따라 OR 방향이 바뀐 기능 — 특정 시기 이벤트 영향 가능성 있음 ⚠️<br>
    · <b>방향 일치율 ≥ 80%</b>: 양호 — 시간적 강건성 확보<br>
    · <b>방향 일치율 &lt; 80%</b>: 주의 — 특정 기간의 이벤트(앱 업데이트, 장애, 마케팅) 영향 검토 필요<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    기간별 OR이 일관되면 "분석 기간의 특수한 이벤트에 의존하지 않는 보편적 결과"라고 주장할 수 있습니다.
    최소 각 기간별 앱당 50건 이상의 리뷰가 있어야 분석이 안정적으로 수행됩니다.
    </div>
    """, unsafe_allow_html=True)
...
def _render_sample_distribution(vr: dict[str, Any]) -> None:
    ...
    <div class="info-box">
    <b>🎯 이 검증이 왜 필요한가?</b><br>
    분석에 투입된 데이터 자체가 편향되면 아무리 정교한 통계도 신뢰하기 어렵습니다.
    "우리가 사용한 데이터가 얼마나 균형 잡혀 있는가"를 시각적으로 확인하는 섹션입니다.<br><br>
    <b>📖 수치를 어떻게 읽는가?</b><br>
    · <b>평점 분포</b>: 1~5점 분포가 고르면 OR 분석이 균형 있게 수행됩니다.
      특정 평점에 리뷰가 몰려 있으면(예: 5점이 90%) 분석 편향 우려가 있습니다.<br>
    · <b>긍정/부정 도넛 차트</b>: 긍정(4~5점)과 부정(1~2점)의 비율을 보여줍니다.<br>
    &nbsp;&nbsp;&nbsp;— ✅ 양호: 긍·부정 비율이 7:3 ~ 3:7 범위<br>
    &nbsp;&nbsp;&nbsp;— ⚠️ 주의: 한쪽이 9:1 이상 — OR 분석에서 소수 클래스 과소 추정 가능<br>
    · <b>월별 추이</b>: 특정 달에 리뷰가 급증하면 그 달의 이벤트(업데이트, 서비스 장애 등)가
      전체 결과를 왜곡하고 있는지 확인이 필요합니다.<br><br>
    <b>💡 결과가 의미하는 바</b><br>
    데이터 불균형이 심하면 로지스틱 회귀에서 과소 표현된 클래스의 OR이 왜곡될 수 있습니다.
    ⚠️ 주의 표시가 있는 앱은 분석 결과 해석 시 "데이터 불균형에 따른 제한점"으로 논문에 명시하세요.
    </div>
    """, unsafe_allow_html=True)
...
def render(
    ...
    <div class="info-box">
    <b>📊 통계 검증이란?</b><br>
    "이 분석 결과를 믿어도 될까?"라는 질문에 답하는 섹션입니다.<br>
    아래 7가지 항목이 모두 통과되면, 논문 심사위원이나 동료 연구자에게
    <b>"우리 분석은 여러 각도에서 검증되었다"</b>고 자신 있게 말할 수 있습니다.<br><br>
    · ✅ <b>양호</b>: 해당 기준을 충족하여 신뢰할 수 있는 수준입니다<br>
    · ⚠️ <b>주의</b>: 기준에 근접하거나 일부 미달 — 결과 해석 시 언급이 필요합니다<br>
    · ❌ <b>경고</b>: 기준 미달 — 데이터 보완 또는 방법론 조정을 권장합니다<br><br>
    각 항목을 펼쳐 세부 결과와 해석을 확인하세요. 📝 논문 기술 예시 박스는 논문에 바로 사용할 수 있는 문장을 제공합니다.
    </div>
    """, unsafe_allow_html=True)
